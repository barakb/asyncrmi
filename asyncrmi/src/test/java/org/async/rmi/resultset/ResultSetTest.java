...
public class ResultSetTest {
    ...
    public void testReadAll() throws Exception {
        ...
        try (ResultSet<Byte> rs = client.retrieve(new File("build.gradle.kts"), 1024)) {
            while (rs.next()) {
                baos.write(rs.get());
            }
        }
        String collected = baos.toString();
        String content = new String(Files.readAllBytes(Paths.get("build.gradle.kts")));
        org.hamcrest.MatcherAssert.assertThat(content, equalTo(collected));
    ...
    public void testOneByte() throws Exception {
        ...
        try (ResultSet<Byte> rs = client.retrieve(new File("build.gradle.kts"), 1024)) {
            ...
            byte firstByte = Files.readAllBytes(Paths.get("build.gradle.kts"))[0];
            logger.info("firstByte is " + firstByte);
            logger.info("read is " + read);
            org.hamcrest.MatcherAssert.assertThat(read, equalTo(firstByte));
        }
    }
